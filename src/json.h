...
// Added an user overridable #define so that the allocator to use can be defined once for the whole program
// by defining the macro 'json_default_alloc' prior to including the header
// TODO Import all tests from https://github.com/sheredom/json.h/tree/master/test
...
#if !defined(json_default_alloc)
#define json_default_alloc(size) malloc(size)
#endif
...
json_parse_ex(const void *src, size_t src_size, size_t flags_bitset,
    ...
    if (json_null == alloc_func_ptr) {
        allocation = json_default_alloc(total_size);
    } else {
...
void *json_write_minified(const struct json_value_s *value, size_t *out_size) {
    ...
    data = (char *)json_default_alloc(size);
